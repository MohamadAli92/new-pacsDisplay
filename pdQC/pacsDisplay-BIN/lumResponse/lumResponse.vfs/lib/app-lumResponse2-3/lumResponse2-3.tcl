package provide app-lumResponse2-3 1.0

###########################################################################
#							lumResponse 2.2b									#
#	Written by:																#
#		Michael J. Flynn (mikef@rad.hfh.edu)								#
#		Philip M. Tchou (pmtchou@engin.umich.edu)							#
...
set title "lumResponse vers. 2.3"
...
puts "        07/2022 - M.J Flynn, P.M Tchou, N.B Bevins"
#
#**********************************************************************
...
#source [file join $apps_path tcl msgboxMod.tsp]     ;# modified message box
...
source [file join $apps_path tcl showManual.tsp]    ;# open a manual pdf section
...
set phase(0) "0	0	0"
set logfile [file join $logDir Lmeter.log]
set log 0
set error 0
set offgrey #606060
...
#
frame .modequit_bar -relief raised -borderwidth 2
frame .spacer_bar -relief raised -borderwidth 2 -height 5
frame .displayID_bar -relief raised -borderwidth 2
frame .init_bar -relief raised -borderwidth 2
frame .init-IL_bar -relief raised -borderwidth 2
frame .rec_bar -relief raised -borderwidth 2
...
frame .lumMeter  -height 140 -width 360 -relief sunken -bg $bgClr \
	           -borderwidth 10 -highlightthickness 1 -highlightbackground $fgClr

label .lumMeter.model -text $model -font {arial 6 bold} \
...
label .lumMeter.outlier -text "OUTLIER" -font {courier 8 bold} \
                        -bg $bgClr -fg $bgClr -padx 1 -pady 1
...
proc ColorRGB {grey} {
	...
	if {$grey >= 0} {
		if {$grey == 255} {
			...
			if {$LUTmode != 0} {
				set red [format "%02.2x" [expr $grey + [lindex $phase($LUTphase) 0]]]
				set green [format "%02.2x" [expr $grey + [lindex $phase($LUTphase) 1]]]
				set blue [format "%02.2x" [expr $grey + [lindex $phase($LUTphase) 2]]]
				set lastRGB #$red$green$blue
			} else {
...
proc IL_rec {w} {
	...
	global record log LUTmode ILstatus
...
#	    to vwait as an indication that a new measurement is in.
			...
			while {$greyR < 256} {
				...
				for {set LUTphase 0} {$LUTphase < $numPhases} {incr LUTphase} {
					...
					if {$greyR <= 0 && $LUTphase == 0} {
						...
						set x 0
						after 1000 {set x 1} ;# force a delay larger than iLdelay
						vwait x
	...
	append textmsg "Would you like to view the log file?"
...
#		... open a file to write the data to
		...
		puts $io_file \
			"#  iLdelay=$iLdelay  avgN=$avgN  ILlimit_plus=$ILlimit_plus ILlimit_minus=$ILlimit_minus  Meter=$meter LUTmode=$LUTmode"
...
#		... option to plot the dL/L data
		if {$LUTmode == 256 || $LUTmode == 52 || $LUTmode == 18 || $LUTmode == 16} {
			set savePlotMsg "Do you wish to analyze the QC (256, 52, 18, or 16x2) saved data?"
		} else {
...
proc ILoutlierTest {} {
	...
	global pause_flag log ILtolerance
...
#
proc getEDID {} {
	global EDIDtxt DisplayName displayN
	...
	if {[catch {exec [file join BIN-LR/getEDID.exe] $displayN} output]} {
